...
uniform samplerCube unif_env_sampler;
...
vec3 calculate_env(vec3 normal_n, vec3 view_direction_n)
{
    vec3 light_reflect_direction_n = normalize(reflect(view_direction_n, normal_n));
    return texture(unif_env_sampler, light_reflect_direction_n).rgb;
}
...
void main()
{
    ...
    vec3 env_color = calculate_env(frag_normal_n, view_direction_n);
